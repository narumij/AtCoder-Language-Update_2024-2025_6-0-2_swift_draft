...
let package = Package(
  ...
  dependencies: [
    ...
    .package(
      ...
      revision: "f8568e0a2d81618894594f77725ebe093fcd2f9b"),
//      exact: "0.1.13"),
    // atcoderでswiftが通用するためには欠かせません
    .package(
...
# 将来このスクリプトをメンテナンスされる方々に於かれましては、
# フルビルドが強制される事態とならないよう尽力されることを切に願います


# 本スクリプトの基本的な方針として
# AtCoderのSwiftユーザーに不利益がないことがあげられます。

# 他に、本スクリプト固有の方針として、以下が上げられます
...
# - 次回の言語アップデートまで、初期の性能を維持できること
#    (運営から開示される条件から品質の低下が予想される場合、適切にアラートを出す)
